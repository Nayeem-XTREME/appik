import mobile1 from '../assets/img/Mobile1.svg'
import mobile2 from '../assets/img/Mobile2.svg'
import mobile3 from '../assets/img/Mobile3.svg'
import mobile4 from '../assets/img/Mobile1.svg'
import mobile5 from '../assets/img/Mobile5.svg'
import mobile6 from '../assets/img/Mobile6.svg'

export const faces = [
  { img: mobile3, alt: 'Mobile 3' },
  { img: mobile4, alt: 'Mobile 4' },
  { img: mobile5, alt: 'Mobile 5' },
  { img: mobile6, alt: 'Mobile 6' },
  { img: mobile1, alt: 'Mobile 1' },
  { img: mobile2, alt: 'Mobile 2' }
]